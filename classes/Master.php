...
Class Master extends DBConnection {
	...
	function save_appointment(){
		...
		if(empty($student_id))
		$sql = "INSERT INTO `students` set name = '{$name}'  ";
		else
		$sql = "UPDATE `students` set name = '{$name}' where id = '{$id}'  ";
		$save_inv = $this->conn->query($sql);
		$this->capture_err();
		if($save_inv){
			$student_id = (empty($student_id))? $this->conn->insert_id : $student_id;
			if(empty($id))
			$sql = "INSERT INTO `appointments` set date_sched = '{$date_sched}',student_id = '{$student_id}',`status` = '{$status}',`message` = '{$message}' ";
			else
			$sql = "UPDATE `appointments` set date_sched = '{$date_sched}',student_id = '{$student_id}',`status` = '{$status}',`message` = '{$message}' where id = '{$id}' ";

			$save_sched = $this->conn->query($sql);
			$this->capture_err();
			$data = "";
			foreach($_POST as $k=> $v){
				if(!in_array($k,array('lid','date_sched','status','message'))){
					if(!empty($data)) $data .=", ";
					$data .= " ({$student_id},'{$k}','{$v}')";
				}
			}
			$sql = "INSERT INTO `student_meta` (student_id,meta_field,meta_value) VALUES $data ";
			$this->conn->query("DELETE FROM `student_meta` where student_id = '{$student_id}'");
	...
	function multiple_action(){
		...
		if($_action != 'delete'){
			...
			$sql = "UPDATE `appointments` set status = '{$status}' where student_id in (".(implode(",",$ids)).") ";
			$process = $this->conn->query($sql);
			$this->capture_err();
		}else{
			$sql = "DELETE a.*,s.*,im.* from  `appointments` a inner join `students` s on a.student_id = s.id  inner join `student_meta` im on im.student_id = s.id where a.student_id in (".(implode(",",$ids)).") ";
			$process = $this->conn->query($sql);
			$this->capture_err();
		}
		if($process){
